...
function setup() {
	...
	background('#FFFFFF');
	window.confirm(
		'Diese Website verwendet Cookies. Einverstanden? Wenn nicht, clicke abbrechen.\n\n[ESC] = Hilfe und Debug Infos'
	);
	frameRate(60);
	strokeWeight(1);
	colorMode(HSL);
...
function draw() {
	...
	for (let i of stars) {
		...
		if (type == 1) {
			...
			for (let i of stars) {
				if (
					i.split('-')[0].split('/')[0] < 55 + screen_width / 2 &&
					i.split('-')[0].split('/')[0] > -55 + screen_width / 2 &&
					i.split('-')[0].split('/')[1] < 55 + screen_height / 2 &&
					i.split('-')[0].split('/')[1] > -55 + screen_height / 2
				) {
				} else {
					if (!out_of_bounds) {
